...
class UltimateLogic(BaseLogic):
    ...
    def diamondInArea(self, position : Position, radius : int):
        blue = [x for x in self.blue_diamonds if (position.x - radius) <= x.x <= (position.x + radius) and (position.y - radius) <= x.y <= (position.y + radius) ]
        red = [x for x in self.red_diamonds if (position.x - radius) <= x.x <= (position.x + radius) and (position.y - radius) <= x.y <= (position.y + radius) ]
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        distance_to_base_portal = self.getDistance(board_bot.position, portal[0]) + self.getDistance(portal[1], base)
        ...
        if props.diamonds == 5 or (board_bot.properties.milliseconds_left/1000 + 3 <= distance_to_base):
            ...
            # if (self.isInDomain(self.goal_position, board_bot.position, board_bot.properties.base)):
            #     self.goal_position = board_bot.properties.base
            print(self.goal_position, board_bot.position, board_bot.properties.base)
